<!-- src/components/modals/salas/Concluidas.vue -->
...
const statusMensagem = computed(() => {
  ...
  if (r.status === 'pendente') {
    return 'Esta reserva ainda está marcada como pendente, mas já foi movida para "Concluídas".'
  }
  if (r.status === 'aprovado') {
    return 'Esta reserva foi aprovada e já foi concluída.'
  }
  return 'Esta reserva foi negada. Reservas negadas aparecem apenas em "Concluídas".'
})
...
const tipoReuniaoLabel = computed(() => {
  const r = props.reserva
  if (!r || !r.tipoReuniao) return ''
  return r.tipoReuniao === 'interna' ? 'Interna' : 'Externa'
})
...
<template>
  <div v-if="open && reserva" class="overlay">
    <div class="card">
      ...
      <div class="content">
        <p class="descricao">
          Veja os detalhes da reserva concluída, incluindo status, horário e participantes.
        </p>
        ...
        <div class="linha-info" v-if="tipoReuniaoLabel">
        ...
        <div class="linha-info" v-if="reserva.horaInicio && reserva.horaFim">
        ...
        <div v-if="reserva.motivo" class="motivo-box">
        ...
        <p v-if="fluxoAvisoInvalido" class="mensagem-erro">
          Atenção: combinação de status e aba pode estar inconsistente
          (pendente não deveria estar em "Concluídas").
        </p>
      </div>
